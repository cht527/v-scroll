...
const STYLE_ID = "v-scroll-global-style",
	...
	TRACK_MARGIN = 3;
const svgToDataUri = (svg_string) => {
	const cleaned_svg = svg_string.replace(/\n/g, "").replace(/\s+/g, " ").trim();
	return `data:image/svg+xml,${encodeURIComponent(cleaned_svg)}`;
};
const injectGlobalStyle = (css) => {
	if (
...
export default class VScroll extends HTMLElement {
	...
	_initCursors = () => {
		const default_cursor = `url('${svgToDataUri(SCROLL_SVG_RAW)}') 12 12, pointer`,
			dragging_cursor = `url('${svgToDataUri(GRAB_SVG_RAW)}') 12 12, grabbing`;
	...
	_handleScroll = () => {
		...
		if (this._scroll_timer) clearTimeout(this._scroll_timer);
	...
	_updateScrollbar = () => {
		...
		if (is_scrollable) {
			...
			this._scroll_ratio =
				max_scroll_top > 0 ? max_thumb_top / max_scroll_top : 0;
			this._thumb.style.height = `${thumb_height}px`;
			this._thumb_height = thumb_height;
			this._updateThumbPosition(scroll_top);
		}
	};
	_updateThumbPosition = (scroll_top) => {
		const thumb_top = this._track_margin + scroll_top * this._scroll_ratio;
	...
	_handlePointerDown = (e) => {
		...
		const grab_cursor = this.style.getPropertyValue(
			"--v-scroll-cursor-dragging",
		);
		if (grab_cursor) document.body.style.cursor = grab_cursor;
	...
	_handleTrackClick = (e) => {
		...
		const rect = this._scrollbar.getBoundingClientRect(),
			...
			target_scroll_top =
				(click_y - this._thumb_height / 2) / this._scroll_ratio;
		this._view.scrollTo({
	...
	_handleKeyDown = (e) => {
		...
		if (e.key === "ArrowDown") new_top += step;
		else if (e.key === "ArrowUp") new_top -= step;
		else if (e.key === "PageDown") new_top += this._view.clientHeight;
		else if (e.key === "PageUp") new_top -= this._view.clientHeight;
		else return;
		e.preventDefault();
